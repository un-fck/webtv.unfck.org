...
const LANG_MAP: Record<string, string> = {
  ...
  ar: 'ar-SA', zh: 'cmn-Hans-CN', ru: 'ru-RU',
};
...
async function pollOperation(operationName: string): Promise<any> {
  for (let i = 0; ; i++) {
    ...
    const op = (await pollRes.json()) as { done?: boolean; response?: any; error?: any; metadata?: any };
    if (op.error) throw new Error(`Chirp operation failed: ${JSON.stringify(op.error)}`);
    if (op.done) return op.response || op;
    if (i % 12 === 11) console.log(`  [Chirp] Still processing... (${(i + 1) * 5}s)`);
  }
}

/** Upload audio to GCS, returns gs:// URI */
async function uploadToGCS(filePath: string, bucket: string, objectName: string): Promise<string> {
...
async function deleteFromGCS(bucket: string, objectName: string): Promise<void> {
  const token = await getAccessToken();
  await fetch(
...
export const googleChirp: TranscriptionProvider = {
  ...
  async transcribe(audioUrl, opts) {
    ...
    const bucket = process.env.GOOGLE_CLOUD_BUCKET || `${projectId}-speech-eval`;

    // Get local audio file
    const ownedPath = !opts?.audioFilePath;
    const filePath = opts?.audioFilePath || await downloadAudioToTemp(audioUrl, 'Chirp');
    ...
    try {
      ...
      execSync(`ffmpeg -y -i "${filePath}" -ac 1 -ar 16000 "${flacPath}"`, { stdio: 'pipe' });
      ...
      console.log(`  [Chirp] Uploading to GCS (${flacSizeMB.toFixed(1)} MB FLAC)...`);
      ...
      console.log(`  [Chirp] Submitting batch recognize (${locale}, chirp_3)...`);
      ...
      for (const useDiarization of [true, false]) {
        ...
        if (useDiarization && errText.includes('diarization')) {
          console.log(`  [Chirp] Diarization not supported for ${locale}, retrying without...`);
          continue;
        }
        await deleteFromGCS(bucket, objectName);
        throw new Error(`Chirp BatchRecognize failed ${batchRes.status}: ${errText}`);
      }
      ...
      if (!transcript?.results) {
        throw new Error(`Chirp returned no results: ${JSON.stringify(result).slice(0, 500)}`);
      }
      ...
      for (const r of transcript.results as any[]) {
        ...
        const startMs = parseFloat(alt.words?.[0]?.startOffset?.replace('s', '') || '0') * 1000;
        const endMs = parseFloat(alt.words?.[alt.words.length - 1]?.endOffset?.replace('s', '') || '0') * 1000;
      ...
      console.log(`  [Chirp] Transcription: ${fullText.length} chars, ${utterances.length} utterances`);
