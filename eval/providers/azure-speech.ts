...
export const azureSpeech: TranscriptionProvider = {
  ...
  async transcribe(audioUrl, opts) {
    const locale = opts?.language ? (LANG_MAP[opts.language] || opts.language) : 'en-US';
    ...
    if (!submitRes.ok) {
      throw new Error(`Azure Speech submit failed ${submitRes.status}: ${await submitRes.text()}`);
    }
    ...
    for (let i = 0; ; i++) {
      ...
      if (jobStatus.status === 'Failed') {
        throw new Error(`Azure Speech job failed: ${JSON.stringify(jobStatus.properties?.error)}`);
      }
      if (i % 12 === 11) console.log(`  [Azure Speech] Still processing... (${(i + 1) * 5}s)`);
    }
    ...
    const resultJson = (await (await fetch(resultFile.links.contentUrl)).json()) as any;
    ...
    const totalDurationMs = utterances.length > 0 ? utterances[utterances.length - 1].end : 0;
