...
async function uploadFile(filePath: string): Promise<{ name: string; uri: string }> {
...
export const gemini: TranscriptionProvider = {
  ...
  async transcribe(audioUrl, opts) {
    ...
    const filePath = opts?.audioFilePath || await downloadAudioToTemp(audioUrl, 'Gemini');
    ...
    try {
              ...
              segments: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    content: { type: 'STRING', description: 'Verbatim transcription of this segment' },
                  },
      ...
      if (!responseText) {
        throw new Error(`Gemini returned empty transcription: ${JSON.stringify(raw.candidates?.[0]?.finishReason)}`);
      }
      ...
      const fullText = segments.map(s => s.content).join(' ').trim();
      const durationMs = utterances.length > 0 ? utterances[utterances.length - 1].end : 0;

      console.log(`  [Gemini] Transcription: ${fullText.length} chars, ${segments.length} segments, ${utterances.length} utterances`);

      // Clean up uploaded file
      await fetch(`${BASE}/v1beta/${file.name}?key=${GEMINI_API_KEY}`, { method: 'DELETE' }).catch(() => {});
