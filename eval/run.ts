...
import { getKalturaAudioUrl, getAvailableAudioLanguages } from '../lib/transcription';
...
import { getProvider, getAllProviders, getProviderNames } from './providers/registry';
...
function parseArgs() {
  ...
  const opts: { symbol?: string; corpus?: string; providers?: string[]; languages?: string[] } = {};

  for (const arg of args) {
    if (arg.startsWith('--symbol=')) opts.symbol = arg.slice('--symbol='.length);
    if (arg.startsWith('--corpus=')) opts.corpus = arg.slice('--corpus='.length);
    if (arg.startsWith('--providers=')) opts.providers = arg.slice('--providers='.length).split(',');
    if (arg.startsWith('--languages=')) opts.languages = arg.slice('--languages='.length).split(',');
  }
  return opts;
}
...
async function evalSession(
  ...
  try {
    ...
    console.error(`  Failed to query audio languages: ${err instanceof Error ? err.message : err}`);
    return [];
  }
  ...
  const targetLangs = (languageFilter || session.languages || Object.keys(UN_LANGUAGES))
    .filter(lang => {
  ...
  for (const lang of targetLangs) {
    ...
    const gtDir = path.join(RESULTS_DIR, 'ground-truth', session.symbol.replace(/\//g, '_'));
    ...
    if (fs.existsSync(gtTextPath)) {
      ...
      try {
        ...
        if (parsed.speakers.length > 0) {
          ...
          fs.writeFileSync(path.join(gtDir, `${lang}_speakers.txt`), speakersText);
        }
        console.log(`  Ground truth: ${groundTruthText.length} chars, ${parsed.speakers.length} speakers → ${gtTextPath}`);
      } catch (err) {
        console.warn(`  Ground truth unavailable: ${err instanceof Error ? err.message : err}`);
      }
    }
    ...
    try {
      ...
      console.error(`  Audio download failed: ${err instanceof Error ? err.message : err}`);
      continue;
    }

    // Run each provider
    const providerOutputs: Record<string, string> = {};
    const rawDir = path.join(RESULTS_DIR, 'raw', session.symbol.replace(/\//g, '_'));
    ...
    for (const providerName of providerNames) {
      ...
      if (fs.existsSync(rawFilePath)) {
        ...
        try {
          const transcript = JSON.parse(fs.readFileSync(rawFilePath, 'utf-8')) as import('./providers/types').NormalizedTranscript;
          providerOutputs[providerName] = transcript.fullText;
          if (groundTruthText) {
            const metrics = computeMetrics(groundTruthText, transcript.fullText, lang);
            results.push({
              symbol: session.symbol, assetId: session.assetId, language: lang, provider: providerName,
              wer: metrics.wer.wer, normalizedWer: metrics.normalizedWer.wer,
              cer: metrics.wer.cer, normalizedCer: metrics.normalizedWer.cer,
              substitutions: metrics.normalizedWer.substitutions, insertions: metrics.normalizedWer.insertions,
              deletions: metrics.normalizedWer.deletions, refLength: metrics.normalizedWer.refLength,
              hypLength: metrics.normalizedWer.hypLength, durationMs: transcript.durationMs,
              timestamp: new Date().toISOString(),
            });
            const r = results[results.length - 1];
            console.log(`    WER: ${(r.wer * 100).toFixed(1)}% | Normalized WER: ${(r.normalizedWer * 100).toFixed(1)}% | CER: ${(r.cer * 100).toFixed(1)}%`);
          }
        } catch (err) {
          console.warn(`    Cache load failed, will re-run: ${err instanceof Error ? err.message : err}`);
      ...
      try {
        ...
        if (groundTruthText) {
          ...
          results.push(result);
          console.log(`    WER: ${(result.wer * 100).toFixed(1)}% | Normalized WER: ${(result.normalizedWer * 100).toFixed(1)}% | CER: ${(result.cer * 100).toFixed(1)}%`);
        } else {
          console.log(`    Transcription complete (${transcript.fullText.length} chars), no ground truth to compare`);
        }
      } catch (err) {
    ...
    if (providerPairs.length >= 2) {
      ...
      for (let i = 0; i < providerPairs.length; i++) {
        for (let j = i + 1; j < providerPairs.length; j++) {
          ...
          console.log(`    ${providerPairs[i]} vs ${providerPairs[j]}: WER ${(pairMetrics.normalizedWer.wer * 100).toFixed(1)}%`);
        }
      }
    }
...
async function main() {
  ...
  const sessions: SessionConfig[] = JSON.parse(fs.readFileSync(sessionsPath, 'utf-8'));
  const filteredSessions = opts.symbol
    ? sessions.filter(s => s.symbol === opts.symbol)
    : sessions;

  if (filteredSessions.length === 0) {
    console.error(`No sessions found${opts.symbol ? ` matching ${opts.symbol}` : ''}`);
    process.exit(1);
  }
  ...
  const existingKey = (r: SessionResult) => `${r.symbol}|${r.language}|${r.provider}`;
