...
export async function resolveSymbol(assetId: string): Promise<ResolvedSession | null> {
  ...
  for (const doc of metadata.relatedDocuments) {
    const symbolMatch = doc.url.match(/[SA]\/(?:\d+\/)?PV\.\d+/) || doc.title.match(/[SA]\/(?:\d+\/)?PV\.\d+/);
    if (symbolMatch) {
  ...
  try {
    ...
    // Extract category
    const categoryMatch = html.match(/<h6[^>]*class="text-primary"[^>]*>([^<]+)<\/h6>/);
    ...
    if (gaMatch && plenaryMatch && /general assembly/i.test(category)) {
      return { symbol: `A/${gaMatch[1]}/PV.${plenaryMatch[1]}`, method: 'titleParsing' };
    }
  } catch {
    // Ignore fetch errors
  }
