...
export async function fetchPVDocument(
  ...
  if (!res.ok)
    throw new Error(
      `Failed to fetch PV document ${symbol} (${language}): ${res.status}`,
    );

  const contentType = res.headers.get("content-type") || "";
  if (!contentType.includes("pdf")) {
    throw new Error(
      `Expected PDF but got ${contentType} for ${symbol} (${language})`,
    );
  }
  ...
  if (!pdfText.includes(symbol)) {
    throw new Error(
      `PDF returned for ${symbol} (${language}) does not contain the expected symbol — wrong document returned by API`,
    );
  }
