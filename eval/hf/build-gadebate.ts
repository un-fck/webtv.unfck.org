...
const S3_BASE = 'https://s3.amazonaws.com/downloads.unmultimedia.org/radio/library/ltd/mp3/ga';
...
const SESSION_YEAR: Record<number, number> = {
  70: 2015, 71: 2016, 72: 2017, 73: 2018,
  76: 2021, 77: 2022, 78: 2023, 79: 2024, 80: 2025,
};
...
async function fetchSpeechEntry(session: number, slug: string): Promise<SpeechEntry | null> {
  ...
  const audioFiles: Partial<Record<'FL' | 'EN' | 'FR' | 'ES' | 'AR' | 'ZH' | 'RU', string>> = {};
  let countryIso = '';
  ...
  const pdfMatch = html.match(/gastatements\/(\d+)\/([a-z]{2,3})_([a-z]{2,3})\.pdf/);
  ...
  const dateMatch = html.match(/(\d{1,2})\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{4})/);
...
async function processSession(gaSession: number): Promise<SpeechEntry[]> {
  ...
  for (let i = 0; i < slugs.length; i += 20) {
    ...
    const results = await Promise.all(batch.map(slug => fetchSpeechEntry(gaSession, slug)));
    for (const r of results) {
      if (r) entries.push(r);
    }
    process.stdout.write(`\r  metadata: ${entries.length}/${Math.min(i + 20, slugs.length)}`);
  }
  ...
  for (const entry of entries) {
    ...
    for (const lang of UN_LANG_CODES) {
      const fileKey = lang === 'FL' ? 'floor_file' : `${lang.toLowerCase()}_file`;
    ...
    process.stdout.write(`\r  audio: ${downloaded} downloaded, ${cached} cached`);
  }
  console.log();
...
async function main() {
  ...
  for (const session of sessions) {
    ...
    fs.writeFileSync(metaPath, allEntries.map(e => JSON.stringify(e)).join('\n') + '\n');
    console.log(`  Checkpoint: ${allEntries.length} total rows → metadata.jsonl`);
  }
