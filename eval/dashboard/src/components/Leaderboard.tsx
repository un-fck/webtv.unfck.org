...
function aggregate(values: number[]): { mean: number; ciLow: number; ciHigh: number } {
  ...
  const t = tCrit(n - 1);
  return { mean, ciLow: Math.max(0, mean - t * se), ciHigh: Math.min(1, mean + t * se) };
}
  ...
  const langBreakdown = useMemo(() => {
    return languages.map(lang => {
      ...
      const providerData = providers
        .map(provider => {
          const values = langResults.filter(r => r.provider === provider).map(r => r[metric]);
  ...
  return (
    <>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        <h3>
          Provider Ranking — Mean {METRIC_LABELS[metric]} (All Languages) with 95% CI
              ...
              {providerData.map(({ provider, mean, ciLow, ciHigh, n }) => {
                ...
                return (
                  <div key={provider} className="bar-row">
                    ...
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-dim)', width: '50px', textAlign: 'right' }}>
                      n={n}
                    </span>
                  </div>
                );
