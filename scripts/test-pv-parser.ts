...
const TEST_DOCS: TestDoc[] = [
  // Security Council
  {
    ...
    label: "SC long debate (Ukraine)",
    minTurns: 10,
  },
  {
    symbol: "S/PV.10100",
    body: "Security Council",
    label: "SC procedural (short)",
    minTurns: 1,
  },
  {
    symbol: "S/PV.10121",
    body: "Security Council",
    label: "SC medium (Afghanistan)",
    minTurns: 5,
  },
  // General Assembly
  {
  ...
  {
    symbol: "A/ES-10/PV.40",
    body: "General Assembly",
    label: "GA emergency special",
    minTurns: 3,
  },
  {
    symbol: "A/C.1/78/PV.7",
    body: "General Assembly",
    label: "GA 1st Committee (PV)",
    minTurns: 3,
  },
  {
    symbol: "A/C.3/78/SR.5",
    body: "General Assembly",
    label: "GA 3rd Committee (SR)",
    minTurns: 3,
  },
...
async function testDocument(doc: TestDoc, lang: string): Promise<TestResult> {
  ...
  try {
    ...
    result.proceduralTurns = parsed.turns.filter(
      (t) => t.type === "procedural",
    ).length;
...
function printResult(r: TestResult) {
  ...
  console.log(
    `       Symbol: ${symbolIcon}  Body: ${bodyIcon}  Turns: ${r.turnCount} ${turnIcon} (min ${TEST_DOCS.find((d) => d.symbol === r.symbol)!.minTurns})`,
  );
  console.log(
    `       President: ${r.presidentFound ? "✅" : "—"}  Members: ${r.membersCount}  Agenda: ${r.agendaCount}`,
  );
  console.log(
    `       Speech: ${r.speechTurns}  Procedural: ${r.proceduralTurns}  LangAnnot: ${r.withLang}`,
  );
  if (r.firstSpeaker)
    console.log(`       First: ${r.firstSpeaker}  Last: ${r.lastSpeaker}`);
}
...
async function main() {
  ...
  const failures = allResults.filter(
    (r) => !r.success && r.error !== "PDF not available",
  );
  ...
  for (const lang of langs) {
    const langResults = allResults.filter(
      (r) => r.lang === lang && r.error !== "PDF not available",
    );
