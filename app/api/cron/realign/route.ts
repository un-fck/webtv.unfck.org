...
/**
 * Cron: realign transcripts whose audio WebTV re-cut after transcription.
 *
...
export async function GET(request: NextRequest) {
  ...
  // Candidates: recent meetings with a recorded source length. We re-check even
  // already-aligned rows (a video can be cut more than once) and rely on the
  // baseline = COALESCE(aligned_duration_ms, source_duration_ms) to self-quiesce
  // rows whose live duration still matches what we reconciled to.
  ...
  if (rows.length === 0) {
    return NextResponse.json({ candidates: 0, shrunk: 0, processed: 0 });
  }

  // Cheap pre-screen: one batched Kaltura duration call, keep rows whose live
  // duration dropped below the length we last reconciled to.
