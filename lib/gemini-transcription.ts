...
import {
  ...
  deleteGeminiFile,
  fmtHHMMSS,
  parseHHMMSSToMs as parseHHMMSS,
  extractJsonObject,
  downloadAudioToTemp,
...
function buildPrompt(
  ...
  if (langCode === "floor") {
    langInstruction =
      "CRITICAL: This is the floor/original audio channel. Speakers may use any language. Transcribe exactly what you hear in whatever language is being spoken. Do NOT translate to any other language.";
...
async function callGeminiOnFile(
  ...
  const { candidatesTokenCount, thoughtsTokenCount, promptTokenCount } =
    usageMetadata;
  console.log(
...
export async function transcribeAudioWithGemini(
  ...
  try {
    ...
    if (audioSeconds > 0) {
      console.log(
        `  [Gemini] Duration: ${Math.round(audioSeconds)}s (${(audioSeconds / 3600).toFixed(2)}h)`,
      );
    }
    ...
    const totalSeconds =
      audioSeconds > 0 ? audioSeconds : CHUNK_THRESHOLD_SECONDS + 1;
    const numChunks = Math.ceil(totalSeconds / CHUNK_DURATION_SECONDS);
    console.log(
      `  [Gemini] Processing ${numChunks} chunk(s) in parallel (${CHUNK_DURATION_SECONDS / 60}min each)...`,
    );
    ...
    for (let i = 0; i < numChunks; i++) {
      ...
      console.log(
        `  [Gemini] Extracting chunk ${i + 1}/${numChunks} (${fmtHHMMSS(startSeconds)}–${fmtHHMMSS(startSeconds + chunkDurationSec)})...`,
      );
      ...
      chunkInfos.map(async (chunk) => {
        ...
        try {
          for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
              console.log(
                `  [Gemini] Starting chunk ${chunk.index + 1}/${numChunks}${attempt > 0 ? ` (retry ${attempt})` : ""}...`,
              );
              const result = await callGeminiOnFile(
                chunk.path,
                options,
                chunk.durationSec,
              );
              const paragraphs = segmentsToOutput(
                result.segments,
                chunk.offsetMs,
              );
              ...
              if (attempt < maxRetries) {
                console.warn(
                  `  [Gemini] Chunk ${chunk.index + 1}/${numChunks} failed (attempt ${attempt + 1}), retrying: ${error instanceof Error ? error.message : error}`,
                );
                continue;
              }
              throw error;
