...
// Anything not listed appears in the "More" overflow popover (without color dot).
const INLINE_CATEGORIES: string[] = [
  "Press Conferences",
  "General Assembly",
  "Security Council",
...
function CategoryFilterRows({
  ...
  // Inline: curated INLINE_CATEGORIES in fixed display order.
  const inlineItems = INLINE_CATEGORIES.filter((c) => optionSet.has(c));
    ...
    <div className="flex flex-wrap gap-1.5">
      ...
      {overflowItems.length > 0 && (
        <Popover>
          ...
          <PopoverContent
            align="start"
            className="max-h-80 w-80 overflow-auto p-3"
          >
            <div className="flex flex-wrap gap-1.5">
...
export function VideoTable({
  ...
  const otherLangsCount = Math.max(0, totalCountIncludingOther - totalCount);
    ...
    return (
      ...
      >
        ...
        {/* Category pill: clicking toggles the matching filter. Hidden on
            ...
            cell. */}
        <td className="hidden py-2.5 pr-2.5 pl-4 align-top sm:table-cell">
          {category ? (
            <CategoryPill
              category={category}
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Schedule: one table per day, with the day as a text heading. The
          extra pt-4 (on top of the root's space-y-6; padding, so it can't
          margin-collapse) gives the first day section the same 40px rhythm
          as the space-y-10 between days. */}
      <div className="pt-4">
        {showEmptyState ? (
        ...
        serverParams.date || isSearchMode ? (
          ...
          <div className="space-y-10">{pastDayGroups.map(renderDayGroup)}</div>
        )}
      </div>
